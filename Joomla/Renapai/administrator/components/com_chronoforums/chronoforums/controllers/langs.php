...
class Langs extends \GCore\Libs\GController {
	function index(){
		$path = \GCore\C::get('GCORE_ADMIN_PATH').'extensions'.DS.'chronoforums'.DS;
		$files = \GCore\Libs\Folder::getFiles($path, true);
		$path = \GCore\C::get('GCORE_FRONT_PATH').'extensions'.DS.'chronoforums'.DS;
		$files = array_merge($files, \GCore\Libs\Folder::getFiles($path, true));
		$strings = array();
